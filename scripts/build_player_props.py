"""
scripts/build_player_props.py  [DEPRECATED — superseded by build_props.py]

This script is a legacy alternative that also generates dashboard/data/player_props.json
but uses season-static data instead of game-specific Pinnacle lines.

build_props.py is the authoritative pipeline script (wired into update.py).
Running this script manually will OVERWRITE the game-specific player_props.json
generated by build_props.py. Only use for debugging/comparison purposes.

Original description: Generates dashboard/data/player_props.json from season averages and game logs.
