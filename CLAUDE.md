...
## What This Is
Python NBA analytics pipeline: data ingestion → feature engineering → game outcome prediction (67.1%, AUC 0.7406) → ATS betting model (54.9%, Brier-optimized) → margin regression model (Ridge, MAE 10.574) → NBAEnsemble (3-model blend) → prediction store → dashboard v3 (Linear/Coinbase redesign: Teams, H2H, Power Rankings, Injuries, Value Bets, Betting Tools, Player Comparison + Props tab) + CLV tracking. Dashboard live at GitHub Pages. Optuna HPO complete: gradient_boosting confirmed best. Odds: Pinnacle guest API (free, keyless).
...
- `scripts/build_live_scores.py` — live scoreboard from nba_api (6 games) -> live_scores.json
- `scripts/build_playoff_odds.py` — 30-team playoff/title odds -> playoff_odds.json
- `scripts/build_streaks.py` — team win/loss streaks + hot/cold players -> streaks.json
- `scripts/build_advanced_stats.py` — 504-player TS%/USG%/ratings -> advanced_stats.json
...
- `dashboard/data/*.json` are COMMITTED to git (not gitignored) — GitHub Pages has no build step; push dashboard/data/ after running update.py to deploy fresh data
