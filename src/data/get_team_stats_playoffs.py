...
def get_team_stats_playoffs(start_year=2000, end_year=2024):
    ...
    for year in range(start_year, end_year + 1):
        ...
        if base_result["success"] and len(base_result["data"]) > 0:
            base_result["data"].to_csv(f"data/raw/team_stats_playoffs/team_stats_playoffs_{season.replace('-', '')}.csv", index=False)
        ...
        if adv_result["success"] and len(adv_result["data"]) > 0:
            adv_result["data"].to_csv(f"data/raw/team_stats_advanced_playoffs/team_stats_advanced_playoffs_{season.replace('-', '')}.csv", index=False)
