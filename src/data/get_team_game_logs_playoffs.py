...
def get_team_game_logs_playoffs(start_year=2000, end_year=2024):
    os.makedirs("data/raw/team_game_logs_playoffs", exist_ok=True)
    ...
    for year in range(start_year, end_year + 1):
        ...
        output_path = f"data/raw/team_game_logs_playoffs/team_game_logs_playoffs_{season.replace('-', '')}.csv"
        data.to_csv(output_path, index=False)
        print(f"  Saved {season} ({len(data)} rows)")


if __name__ == "__main__":
    get_team_game_logs_playoffs()
